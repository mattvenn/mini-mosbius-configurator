...
docker run --rm -v "$PWD:/work" -w /work/xschem/mosbius_lib \
  ...
  'export PDK=sky130A PDK_ROOT=/foss/pdks
   xschem --rcfile $PDK_ROOT/sky130A/libs.tech/xschem/xschemrc -n -q -o /work/build your_design.sch'
...
python3 -m mosbius.cli route build/your_design.spice --out build/your_design.mosbius.json
...
`mosbius watch build/your_design.spice` re-runs route+check every time
xschem re-netlists the file (polls the file's mtime, so it works across the
...
The Python tooling has no external dependencies beyond pytest, but pytest
isn't installed on the host in this project's dev setup -- run it inside the
same container:

```bash
docker run --rm -v "$PWD:/work" -w /work hpretl/iic-osic-tools:latest --skip bash -lc \
...
- `build/` is gitignored -- it's regenerated by netlisting/routing, never
