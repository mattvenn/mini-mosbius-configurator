...
# Monthly regression check for examples/pdiffamp: netlists it, routes it,
# builds the routed subcircuit, runs the real tb_pdiffamp.sch testbench
# through ngspice, and checks the gains and the operating point land near
# the reference measurements in examples/pdiffamp/README.md via
# check_pdiffamp_sim.py. This is what
# .github/workflows/spice-regression.yml runs once a month, alongside the
# inverter, ring, SR latch, diff amp, OTA follower and current source
# checks.
#
...
#       --skip bash -lc 'sh tools/check_pdiffamp_sim.sh'
#
# Budget a couple of minutes: sky130A's model library takes 1-2 minutes to
# parse whatever the circuit is, and this deck runs a 6.5us transient --
# the output is a high-impedance node (tens of kOhm) against a 10pF probe,
# so each input level has to be held for microseconds before it is settled
# enough to measure.
set -e
...
echo "== netlisting, routing and building examples/pdiffamp/pdiffamp.sch"
sh tools/regenerate_routed.sh examples/pdiffamp/pdiffamp.sch

echo "== netlisting examples/pdiffamp/tb_pdiffamp.sch"
netlist_schematic examples/pdiffamp/tb_pdiffamp.sch

echo "== running ngspice"
cp .spiceinit build/.spiceinit
( cd build && ngspice -b tb_pdiffamp.spice > ngspice_tb_pdiffamp.log 2>&1 ) \
    || { echo "ngspice exited non-zero -- tail of build/ngspice_tb_pdiffamp.log:"; tail -40 build/ngspice_tb_pdiffamp.log; exit 1; }

python3 tools/check_pdiffamp_sim.py build/ngspice_tb_pdiffamp.log
