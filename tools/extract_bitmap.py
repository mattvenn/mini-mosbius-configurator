...
"""Regenerate mosbius/chips/tnt_bits.py from the submodule and the generated netlist.
...
BITMAP_OUT = REPO_ROOT / "mosbius" / "chips" / "tnt_bits.py"
...
# ---------------------------------------------------------------------------
# Step 6: emit mosbius/chips/tnt_bits.py
# ---------------------------------------------------------------------------

HEADER = '''# SPDX-License-Identifier: Apache-2.0
"""Generated by tools/extract_bitmap.py -- do not edit by hand.

Maps every one of the 192 config-chain bits of tnt's mini-MOSbius
(`tt_um_tnt_mosbius`, `ctrl_out[191:0]`, see SPEC.md Sec 2.1-2.2) to the
`mosbius.sym` pin it drives, and for the 162 bits that close a switch, to the
crosspoint/bus/row it closes.
...
Andrew Kang's part has its own table next door in `kang_bits.py`. The two are
joined to everything else by `mosbius/chips/__init__.py`.
...
def main():
    ...
    ap.add_argument("--write", action="store_true", help="write mosbius/chips/tnt_bits.py")
    args = ap.parse_args()
