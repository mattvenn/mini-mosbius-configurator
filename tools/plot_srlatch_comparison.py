...
Inputs, all produced by other commands so this script only draws:
    ...
    build/srlatch_tb_out_routed.txt   tools/sim/check_example_sim.sh srlatch
...
def main() -> None:
    for path, how in [
        (DRAWN, "Run `sh tools/sim/check_example_sim.sh srlatch` in the IIC-OSIC-TOOLS container "
                "to\n  simulate tb_srlatch.sch and write it."),
        (ROUTED, "Run `sh tools/sim/check_example_sim.sh srlatch` in the IIC-OSIC-TOOLS container "
                 "to\n  simulate tb_srlatch.sch and write it."),
