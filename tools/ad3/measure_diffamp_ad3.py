...
(`ua1`, `ua2`), output on pad G (`ua4`), bias current into pad K.
`--project tt_um_mosbius` measures the same schematic routed for Andrew
Kang's part instead, on different pads -- the wiring table and pad picture
are derived from whichever `--project` is running, not memorised. Run from
the repo root, on the host:

    python3 tools/ad3/measure_diffamp_ad3.py
    python3 tools/ad3/measure_diffamp_ad3.py --project tt_um_mosbius
...
# the chip.
PROJECTS = {
    # examples/diffamp as the router placed it on 2026-08-29 -- the
    # configuration the measured gain and its bias sweep were taken with.
    # SIM is examples/diffamp/README.md, re-run 2026-08-29 at cprobe=10p,
    # rprobe=10meg. NOTE those are 10 MOhm / 10 pF; an AD3 is 1 MOhm / 24 pF,
    # and 1 MOhm across this amp's ~20 kOhm output is worth a couple of
    # percent of gain. The comparison in report() is not corrected for that.
    "tt_um_tnt_mosbius": {
        "chip": TNT,
        ...
        "confirmed_note": "ua1->C, ua2->J and ua0->K have",
    },
    # examples/diffamp routed for tt_um_mosbius on 2026-09-08. SIM is from
    # the same tb_diffamp.sch +-40mV chord this script measures, simulated
    # with Andrew Kang's device library and geometry -- not an independent
    # near-origin fit the way tnt's sim_small_signal is, since none has been
    # run for this part yet. It lands within 0.1 V/V of tnt's own chord
    # gains, which is what the one-library-either-part branch predicts:
    # this circuit's operating point depends on total device width, which
    # is identical on both parts.
    "tt_um_mosbius": {
        "chip": KANG,
        "bitstream": "0804c08008080000000000000823100003020200000008000",
        "sim": {"drawn": {"base": 2.013, "plus40": 2.745, "minus40": 1.238,
                          "gain_plus": 18.30, "gain_minus": 19.36},
                "routed": {"base": 2.018, "plus40": 2.763, "minus40": 1.236,
                           "gain_plus": 18.63, "gain_minus": 19.53}},
        "sim_small_signal": 19.36,  # the as-drawn gain_minus chord above
        "confirmed_note": "ua1->K and ua2->C have (examples/inverter, 2026-09-08)",
    },
}
...
def program_chip(project: str, bitstream: str, chip, port: str | None) -> None:
    ...
    print(f"== loading the differential amplifier onto the chip ({project})")
    try:
...
def main() -> None:
    ...
    out_path = f"build/diffamp{'_kang' if args.project == 'tt_um_mosbius' else ''}_silicon.json"
    ...
    with ad3.device() as handle:
        ...
        if vout0 < 0.2 or vout0 > 3.1:
            print(f"\n  THE OUTPUT IS AT A RAIL ({vout0:.3f} V), so there is nothing\n"
                  ...
                  f"  silicon -- {spec['confirmed_note']}. Either it is the wrong\n"
...
def report(record, spec) -> None:
    ...
    print(f"    output base  {sim['drawn']['base']:5.3f}      {sim['routed']['base']:5.3f}"
          f"       {nominal[3][1]:5.3f} V   (at the peak-gain point)")
