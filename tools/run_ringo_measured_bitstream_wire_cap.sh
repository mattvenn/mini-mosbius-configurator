#!/usr/bin/env bash
# Same as tools/run_ringo_measured_bitstream.sh (the exact measured
# bitstream, 380088007001000010000404250109000400000040000014, real
# pads on all 3 loop pins, row-coupling cap on all 150 matrix switches --
# that script got 42.92MHz vs real silicon's ~30MHz), PLUS real
# horizontal bus-wire self-capacitance to substrate on all 5 bus rows
# this bitstream uses. This is a genuinely different, longer-distance
# effect than the row-coupling cap (which is local, within one column) --
# it's the actual physical metal trace running a bus row's full length
# across the matrix (up to 26 columns), extracted via a full magic PEX of
# ttsky-mini-mosbius/mag/asw_matrix.mag (not just one column). Real
# values found: 865-910fF for most rows, 1.82pF for bus_B[2] -- roughly
# 20-40x bigger than an earlier, never-verified hand-estimate (~30-50fF)
# that had been the reason this was skipped until now. See this script's
# body for the extraction method and per-row values.
#
...
# tnt's own bring-up configuration), not tb_mosbius_ringo.sch's own different
...
* silicon (tnt's bring-up configuration) -- NOT tb_mosbius_ringo.sch's own
...
* Real horizontal bus-wire self-capacitance to substrate, extracted from
* a full magic PEX of ttsky-mini-mosbius/mag/asw_matrix.mag (the whole
* 26-column matrix, not just one column like the row-coupling cap used --
* this is the actual physical metal trace running the row's full length,
* a completely different, longer-distance effect). Never included in any
...
.control
   ...
   set temp = 27
   tran 100p 500n UIC
   wrdata ringo_wire_cap_ua1_pad.txt v(ua1_pad)
   wrdata ringo_wire_cap_ua2_pad.txt v(ua2_pad)
   wrdata ringo_wire_cap_ua4_pad.txt v(ua4_pad)
   wrdata ringo_wire_cap_ua1_net.txt v({UA1_ALIAS})
   wrdata ringo_wire_cap_ua2_net.txt v({UA2_ALIAS})
   wrdata ringo_wire_cap_ua4_net.txt v({UA4_ALIAS})
.endc

"""

library = open("build/mosbius_device_library.spice").read()
with open("build/ringo_measured_wire_cap.spice", "w") as f:
    f.write(top + library)

print(f"Wrote build/ringo_measured_wire_cap.spice ({len(top.splitlines()) + len(library.splitlines())} lines)")
PYEOF

echo "== Sanity-checking the generated netlist =="
python3 <<'PYEOF'
path = "build/ringo_measured_wire_cap.spice"
text = open(path).read()
...
for wr in ("wrdata ringo_wire_cap_ua1_pad.txt v(ua1_pad)",
           "wrdata ringo_wire_cap_ua2_pad.txt v(ua2_pad)",
           "wrdata ringo_wire_cap_ua4_pad.txt v(ua4_pad)",
           "wrdata ringo_wire_cap_ua1_net.txt v(ua1_net)",
           "wrdata ringo_wire_cap_ua2_net.txt v(ua2_net)",
           "wrdata ringo_wire_cap_ua4_net.txt v(ua4_net)"):
    assert wr in text, f"wrdata line missing: {wr}"

print(f"OK: {n_caps} coupling caps present, {n_subckt} subckt/.ends pairs balanced, "
      f"3 pad instances present, 5 bus-wire caps present, no bracketed net in .control, "
      f"6 wrdata calls present.")
PYEOF
...
for pin, alias in (("ua1", "ua1"), ("ua2", "ua2"), ("ua4", "ua4")):
    report(f"{pin}_pad (external, pad-loaded)", f"build/ringo_wire_cap_{alias}_pad.txt")
    report(f"{pin}_net (internal, pre-pad)", f"build/ringo_wire_cap_{alias}_net.txt")

print("Compare against: 42.92MHz (tools/run_ringo_measured_bitstream.sh -- same "
      "bitstream/pads/row-coupling cap, but no bus-wire capacitance), real "
      "silicon ~30MHz (this exact bitstream).")
PYEOF
