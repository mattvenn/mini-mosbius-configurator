...
The +-25% band is deliberately generous: this is a monthly canary for the
toolchain (docker image updates, PDK changes, our own code) landing in the
right regime, not a golden-number match -- the inverter README notes
reltol=0.01 keeps repeat runs stable to well under 0.1%, so a real break
(wrong device library, broken routing, a reintroduced zero-ohm tie) blows
straight past this band rather than sitting near its edge.
"""
