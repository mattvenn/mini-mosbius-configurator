...
`inverter.sch` was drawn by hand in xschem on 2026-08-21, replacing a
version generated by `tools/gen_example_schematic.py` against the
pre-redraw symbol geometry. It is built on `minimosbius_template.sch` from
...
```
$ python3 -m mosbius.cli route xschem/mosbius_lib/simulation/inverter.spice
