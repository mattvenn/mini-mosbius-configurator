...
schematic="tcleval([file normalize examples/pdiffamp/pdiffamp.sch])"}
C {mini_mosbius.sym} -60 -120 0 0 {name=x2
schematic=pdiffamp_routed
spice_sym_def="tcleval([mosbius_routed_include build/pdiffamp_routed.spice])"
tclcommand="textwindow [file normalize build/pdiffamp_routed.spice]"}
...
C {devices/vsource.sym} -130 160 0 0 {name=Vinp value="PWL(0 1.5 999n 1.5 1000n 1.51 3499n 1.51 3500n 1.49 5999n 1.49 6000n 1.5)"}
...
.control
  ...
  let gain_drawn_pos  = (vout_drawn_pos - vout_drawn_base)/0.01
  let gain_drawn_neg  = (vout_drawn_base - vout_drawn_neg)/0.01
  let gain_routed_pos = (vout_routed_pos - vout_routed_base)/0.01
  let gain_routed_neg = (vout_routed_base - vout_routed_neg)/0.01
  print gain_drawn_pos gain_drawn_neg gain_routed_pos gain_routed_neg
  wrdata pdiffamp_tb_inp.txt v(inp)
  wrdata pdiffamp_tb_out_drawn.txt v(out_drawn)
  wrdata pdiffamp_tb_out_routed.txt v(out_routed)
  write tb_pdiffamp.raw

.endc
"}
C {devices/launcher.sym} 210 -280 0 0 {name=h5
descr="load waves"
tclcommand="xschem raw_read $netlist_dir/tb_pdiffamp.raw tran"
}
C {devices/launcher.sym} 210 -240 0 0 {name=h6
descr="generate routed spice"
tclcommand="execute 1 sh tools/regenerate_routed.sh examples/pdiffamp/pdiffamp.sch"
}
