...
def generate_device_script(
    ...
    project, set ibias, shift `config`'s chain MSB-first with enable held low
    throughout, then raise enable. Prints one JSON result line.

    The chain length comes from the part: tnt's is 192 bits, Andrew Kang's is
    196. Shifting the wrong number is not a near miss -- every bit lands one
    position out, so the whole configuration is something else.
    """
    chip = config.chip
    bit_string = "".join(
        "1" if bit in config.bits else "0" for bit in range(chip.num_bits - 1, -1, -1)
    )
    ibias_level = _ibias_level(config.ibias)
    reset_via = chip.reset_via
...
{_IDENTITY_SNIPPET}
BITS = "{bit_string}"  # MSB first: BITS[0] is the last chain bit, BITS[-1] is bit 0
# Where this design takes its reset from. tnt's wrapper passes the Tiny
# Tapeout harness reset through to the chain; Andrew's leaves that
# unconnected and uses its own ui_in[2] instead, so reset_project() does
# nothing there. Driving the wrong one is silent: the chain never clears.
RESET_VIA = {reset_via!r}
PROJECT = {project!r}
DO_RESET = {reset!r}
DO_VERIFY = {verify!r}
...
try:
    ...
    if RESET_VIA == "ui2":
        # Active low, and shared with nothing else, so it has to be held
        # high for the whole shift as well as pulsed here.
        tt.ui_in[2] = 1
        if DO_RESET:
            tt.ui_in[2] = 0
    ...
    # -- Sec 3.5 step 3: shift the whole chain, MSB first. enable stays low.
    ...
    if DO_VERIFY:
        # Sec 3.5's --verify: shift the same bits again, sampling data_out
        # (uo_out[0]) BEFORE each clock -- the chain is exactly as deep as
        # the string is long, so what comes out now is what went in above.
        tt.ui_in[1] = 0
        captured = []
        for bit_char in BITS:
...
def program(
    ...
    *,
    ...
    # The configuration knows which part it was routed for, and --project
    # says which part is being programmed. If those disagree, the bits are
    # about to be shifted into a chip whose chain means something else
    # entirely -- not a slightly wrong circuit, an unrelated one. This is the
    # last point at which anything can tell.
    if project != config.chip.macro:
        raise ProgramError(
            messages.PROGRAM_WRONG_CHIP.format(
                project=project, routed_for=config.chip.macro,
                routed_title=config.chip.title,
            )
        )
