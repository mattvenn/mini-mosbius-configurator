...
It also carries one line for the *other* half of that testbench: a global
`.param pmos_width_per_finger`, written above the subcircuit, which the
ideal `mosbius_*` symbols on the design sheet divide their total widths by
to get a finger count. That is the only geometry difference between the two
taped-out parts, and putting it here means `--project` is the single place a
part is ever chosen -- the as-drawn and as-routed halves of a comparison
cannot end up being different chips. See `render_drawn_geometry()`.
...
def render_drawn_geometry(chip: Chip) -> str:
    """The part's device geometry, as a global `.param`, for the *as drawn*
    half of a testbench.

    This lands outside the subcircuit below, so it is global to the whole
    deck, which is what lets it reach the ideal `mosbius_*` symbols on the
    design sheet as well as the switch matrix here. It is written into the
    generated netlist rather than set on the testbench so that the part
    named by `mosbius route --project` is the only place a part is ever
    ...
    return (
        ...
        f"* design sheet pick it up too, and the drawn and routed halves of a\n"
        f"* comparison cannot disagree about which part they are.\n"
        f"*\n"
        f"* Every PMOS has the same total width and length on both taped-out\n"
...
def render_mosbius_wrapper(config: SwitchConfig, name: str) -> str:
    ...
    header = (
        render_drawn_geometry(config.chip) + "\n"
...
def simulate_from_routed_json(path: Path, chip: Chip = DEFAULT_CHIP) -> tuple[str, str]:
    ...
    # `mosbius route` records which part it routed for, so a mismatch here is
    # answerable rather than a guess. It matters most when the two parts have
    # the same chain length, where nothing downstream would notice: the
    # bitstream would unpack happily and mean something else entirely. Files
    # written before that field existed carry no claim, so they are accepted.
    stored_project = data.get("project")
    if stored_project is not None and stored_project != chip.macro:
        raise SimulateError(
            messages.SIMULATE_WRONG_PROJECT.format(
                path=path, stored=stored_project, asked=chip.macro)
        )
